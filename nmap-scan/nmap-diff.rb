...
def common_open_port_check(new_gnmap, rspec = false)
  ...
  new_gnmap.each do |host|
    ...
    port_list.each do |port_num|
      ...
      break if port_check_boolean == true
    end

    next unless port_check_boolean
...
def clean_up_port_info(host)
  ...
  port_array.each do |port_string|
    clean_ports.push(
      'port' => port_string.split(%r{/})[0],
      'state' => port_string.split(%r{/})[1],
      'protocol' => port_string.split(%r{/})[2],
      'service' => port_string.split(%r{/})[4]
    )
  end
  clean_ports
end
...
def get_aws_instance_info(host)
  File.open("#{Dir.pwd}/output/all-instances.csv").grep(/#{host}/)
