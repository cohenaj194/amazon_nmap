...
def grab_scannable_instances(address_blob, active_addresses)
  ...
  address_blob.each do | address |
    ...
    if active_addresses.include?(address['public_ip_address'])
      ec2_addresses_list.push(address['public_ip_address'])
    end
  end
  return ec2_addresses_list 
end
...
def scan_ports(region,ec2_addresses_list)
  begin
    ...
    check_exit_status($?.exitstatus, scan_results)
    ...
    %w(
      ...
      scan_results=`#{SSH_COMMAND} centos@#{NMAP_INSTANCE_ADDRESS} 'cd /home/centos/scans/ && cat output.#{file_type}' > #{File.expand_path File.dirname(__FILE__)}/output/#{file_name}`
      check_exit_status($?.exitstatus, scan_results)
    end
...
def main
  ...
  use_ec2_addresses_hash(@options[:json_path]).each do | region, address_blob |
    ...
    if ec2_addresses_list != []
      scan_ports(region,ec2_addresses_list)
    end
  end
