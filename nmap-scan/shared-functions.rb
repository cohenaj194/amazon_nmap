...
def aws_regions_list(access,secret)
  return create_ec2_client(access,secret,"us-west-2").describe_regions.regions.map { |record| record['region_name'] }
end
...
def create_s3_client(access,secret)
  Aws::S3::Client.new(
    region: "us-west-2",
...
def get_ip_list_from_hash(ec2_addresses_hash)
  ...
  ec2_addresses_hash.each do | region, address_blob |
